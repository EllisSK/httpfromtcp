...
func TestValidSingleHeaderWithWhitespace(t *testing.T) {
	headers := NewHeaders()
	data := []byte("Host:           localhost:42069          \r\n\r\n")
	n, done, err := headers.Parse(data)
	require.NoError(t, err)
...
func TestValidTwoHeadersWithExistingHeaders(t *testing.T) {
	...
	require.NotNil(t, headers)
	assert.Equal(t, "localhost:42069", headers["host"])
	assert.Equal(t, "example", headers["content"])
	assert.Equal(t, 43, n)
	assert.True(t, done)
}

func TestInvalidDone(t *testing.T) {
	headers := NewHeaders()
	data := []byte("Host: localhost:42069\r\n")
	n, done, err := headers.Parse(data)
	require.NoError(t, err)
...
func TestInvalidHeaderChars(t *testing.T) {
	headers := NewHeaders()
